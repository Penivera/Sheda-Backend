...
from app.schemas.user_schema import UserInDB,AgentFeed,AccountInfoBase
from app.models.property import Property,Appointment,AgentAvailability,PaymentConfirmation,Contract,AccountInfo
...
async def update_agent_availabilty(update:AgentAvailabilitySchema,db:AsyncSession,schedule_id:int,current_user:AgentFeed):
    schedule = next((availability for availability in current_user.availbilities if availability.id == schedule_id),None)
    if not schedule:
        raise HTTPException(
            status_code = status.HTTP_404_NOT_FOUND,
...
async def run_create_contract(
    ...
    if property.status != PropertyStatEnum.available:
        raise HTTPException(status_code=400, detail="Property is not available for contract")

    # Ensure current user is a client and not the agent
    if property.agent_id == current_user.id:
        raise HTTPException(status_code=400, detail="Agents cannot create contracts for their own properties")

    # Check if payment is marked as completed
    if not contract_data.is_payment_made:
        raise HTTPException(status_code=400, detail="Payment must be completed before creating a contract")
    ...
    property.status = PropertyStatEnum.rented if contract_type == ListingTypeEnum.rent else PropertyStatEnum.sold
